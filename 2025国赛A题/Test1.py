...
【本版要点（含“固定速度”增强）】
1) 仅最大化“遮掩（覆盖）时间”，不再考虑“间隔时间 G”。
2) “固定航向 + 固定速度 + 同航向增产”：每架 UAV 一旦首枚被选中，后续该 UAV 的
   候选必须保持“同航向(±heading_tol_deg)”且“同速度(±speed_tol_mps)”，并沿此航向与速度批量扩展。
...
def _feasible_with(
    ...
    fixed_speed_per_uav: bool,
    speed_tol_mps: float
) -> bool:
    """配额/投放间隔/固定航向/固定速度 约束"""
    u = cand["uav"]
    ...
    if fixed_heading_per_uav:
        ...
        if theta_refs:
            if _ang_diff(cand["theta"], theta_refs[0]) > heading_tol_rad:
                return False
    # 速度锁定
    if fixed_speed_per_uav:
        v_refs = [c["v"] for c in chosen if c["uav"]==u]
        if v_refs:
            if abs(cand["v"] - v_refs[0]) > speed_tol_mps:
                return False
    return True
...
def augment_heading_locked_candidates(
    ...
    v_ref: Optional[float] = None,         # ← 新增：若锁速则只用该速度
    dedup_eps: float = 0.10
) -> List[Dict[str,Any]]:
    """在固定航向(θ_ref)上扫描 t_burst/tau/(v 或 v_ref)，保留覆盖>0 的条目并去重。"""
    u0 = UAVS[uav_idx]["U0"]
    ...
    # 若锁速：仅用 v_ref
    v_candidates = [v_ref] if (v_ref is not None) else list(v_list)
...
def greedy_select_Q5(
    ...
    fixed_speed_per_uav: bool = True,     # ← 新增
    speed_tol_mps: float = 0.0,           # ← 新增（默认严格锁速）
    ...
    while len(chosen) < total_budget:
        ...
        for c in remain:
            if not _feasible_with(
                c, chosen, budgets, min_drop_gap,
                fixed_heading_per_uav, tol_rad,
                fixed_speed_per_uav, speed_tol_mps
            ):
                continue
        ...
        # 首次选中该 UAV → 同航向（与可选“同速度”）增产
        if fixed_heading_per_uav and first_pick:
            theta_ref = best["theta"]
            v_ref = best["v"] if fixed_speed_per_uav else None
            ...
            def ok_interval(cnew):
                for csel in chosen:
                    if csel["uav"]==u and abs(csel["t_drop"] - cnew["t_drop"]) < min_drop_gap:
                        return False
                return True
            # 若锁速：二次过滤速度一致（尽管 augment 已用 v_ref）
            def ok_speed(cnew):
                return (not fixed_speed_per_uav) or (abs(cnew["v"] - best["v"]) <= speed_tol_mps)
            extra = [e for e in extra if ok_interval(e) and ok_speed(e)]
            remain.extend(extra)
...
def gap_fill_phase(
    ...
    heading_tol_deg: float = 2.0,
    fixed_speed_per_uav: bool = True,     # ← 新增
    speed_tol_mps: float = 0.0,           # ← 新增
    force_use_all: bool = True
):
    """按最大空档导向补射，仍遵守固定航向与固定速度等约束；评分只看覆盖增益"""
    ...
    for m_idx, _ in enumerate(MISSILES):
        ...
        while improved and trial_times < 3:
            ...
            for u in range(len(UAVS)):
                ...
                if not _feasible_with(
                    synth, chosen, budgets, min_drop_gap,
                    fixed_heading_per_uav, tol_rad,
                    fixed_speed_per_uav, speed_tol_mps
                ):
                    continue
...
def polish_L1(selected: List[Dict[str, Any]], tgrids: Dict[str, np.ndarray],
              ...
              fixed_heading_per_uav: bool = True,
              heading_tol_deg: float = 2.0,
              fixed_speed_per_uav: bool = True,
              speed_tol_mps: float = 0.0):
    ...
    # 每 UAV 的参考航向/速度（按当前 best）
    ref_theta = {}
    ref_speed = {}
    for c in best:
        u = c["uav"]
        if u not in ref_theta: ref_theta[u] = c["theta"]
        if u not in ref_speed: ref_speed[u] = c["v"]
    ...
    for _ in range(rounds):
        ...
        for i in range(len(best)):
            ...
            c = best[i]
            # 航向候选：若锁航向，仅允许在 tol 内微调；否则小范围搜索
            thetas = [c["theta"] + d for d in (0.0, math.radians(-2), math.radians(2))]
            if fixed_heading_per_uav:
                thetas = [th for th in thetas if _ang_diff(th, ref_theta[c["uav"]]) <= tol_rad]
            # 速度候选：若锁速度，仅允许原速（±speed_tol 内视作同速）；否则小范围搜索
            if fixed_speed_per_uav:
                vs = [c["v"]]  # 不改速
            else:
                vs = [clip(c["v"] + dv, 70.0, 140.0) for dv in (-6.0, 0.0, 6.0)]
            if fixed_speed_per_uav:
                vs = [v for v in vs if abs(v - ref_speed[c["uav"]]) <= speed_tol_mps]

            tds    = [clip(c["t_drop"] + dt, 0.0, 60.0) for dt in (-1.0,-0.5,0.0,0.5,1.0)]
            taus   = [clip(c["tau"] * r, 0.2, 12.0) for r in (0.9,1.0,1.1)]
...
def solve_q5_graph(
    ...
    fixed_heading_per_uav: bool = True,
    heading_tol_deg: float = 2.0,
    fixed_speed_per_uav: bool = True,     # ← 新增：默认锁速
    speed_tol_mps: float = 0.0,           # ← 新增：默认严格不许变速
    force_use_all: bool = True
) -> Dict[str, Any]:
    # 1) 候选（分数=覆盖时长）
    ...
    chosen, union_masks = greedy_select_Q5(
        ...
        fixed_speed_per_uav=fixed_speed_per_uav,
        speed_tol_mps=speed_tol_mps,
        force_use_all=force_use_all
    )

    # 3) gap 兜底（可选，遵守固定航向/速度）
    if do_gap_fill:
    ...
    # 4) L1 精修（可选，仍以覆盖为目标；尊重固定航向/速度）
    if do_polish_L1:
        chosen, union_masks = polish_L1(
            chosen, tgrids, dt_mask, N_ANG, N_Z, INCLUDE_SIDE, rounds=2,
            fixed_heading_per_uav=fixed_heading_per_uav,
            heading_tol_deg=heading_tol_deg,
            fixed_speed_per_uav=fixed_speed_per_uav,
            speed_tol_mps=speed_tol_mps
    ...
    return {
        "method": "graph_Q5",
        "mode": f"L0{' + L1_polish' if do_polish_L1 else ''} (only coverage, heading+speed locked)",
        "selected": out_sel,
        "rows": rows,
...
if __name__ == "__main__":
    ans = solve_q5_graph(
        ...
        heading_tol_deg=2.0,
        fixed_speed_per_uav=True,      # ★ 固定速度（锁定后不可变）
        speed_tol_mps=0.1,             # 速度严格一致（如需容差，可设 0.5 等）
